...
export async function decodeMBP2(imageFile) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const reader = new FileReader();
    ...
    reader.onload = (e) => {
      img.onload = () => {
        try {
          ...
          // Generate same pseudorandom order
          const order = shuffleArray(nPixels, SEED);

          // Extract LSBs from blue channel
          const bits = new Uint8Array(nPixels);
          for (let i = 0; i < nPixels; i++) {
            const pixelIdx = order[i];
            const blueIdx = pixelIdx * 4 + 2;
            bits[i] = pixels[blueIdx] & 1;
          }

          // Convert bits to bytes
          const nBytes = Math.floor(bits.length / 8);
          const payloadBytes = new Uint8Array(nBytes);
          ...
          // Parse header
          if (payloadBytes.length < HEADER_SIZE) {
            reject(new Error('Image too small or not a valid MBP2 stego image'));
            return;
          }

          // Check magic bytes
          const magic = payloadBytes.slice(0, 4);
          if (magic[0] !== 0x4D || magic[1] !== 0x42 || magic[2] !== 0x50 || magic[3] !== 0x32) {
            reject(new Error('Magic bytes not found. Not a valid MBP2 image.'));
            return;
          }

          // Read payload length (little-endian)
          const payloadLen = payloadBytes[4] | (payloadBytes[5] << 8) |
                            (payloadBytes[6] << 16) | (payloadBytes[7] << 24);

          // Read CRC32 (little-endian)
          const expectedCrc = (payloadBytes[8] | (payloadBytes[9] << 8) |
                              (payloadBytes[10] << 16) | (payloadBytes[11] << 24)) >>> 0;

          // Extract payload
          if (HEADER_SIZE + payloadLen > payloadBytes.length) {
            reject(new Error('Payload length mismatch'));
            return;
          }

          const payload = payloadBytes.slice(HEADER_SIZE, HEADER_SIZE + payloadLen);

          // Verify CRC32
          const actualCrc = crc32(payload);
          if (actualCrc !== expectedCrc) {
            reject(new Error(`CRC32 mismatch. Expected 0x\${expectedCrc.toString(16)}, got 0x\${actualCrc.toString(16)}`));
            return;
          }
          ...
          try {
            ...
            reject(new Error('Failed to decompress payload: ' + err.message));
          }
